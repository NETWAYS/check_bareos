...
def checkEmptyTapesPool(courser, warning, critical, poolname):
        checkState = {}
        query_template = """
         SELECT Count(MediaId)
          FROM Media,Pool,Storage
          ...
          AND (VolStatus like 'Purged' or VolStatus like 'Recycle' or lastwritten+(media.volretention * '1 second'::INTERVAL)<now() and VolStatus not like 'Error')
        """
        query = query_template + """AND Pool.name='""" + poolname + """';"""
        courser.execute(query)
        results = courser.fetchone()  # Returns a value
        result = float(results[0])
...
def argumentParser():
    ...
    tapeGroup.add_argument('-ep', '--emptyTapesPool', dest='emptyTapesPool', action='store', help='Count empty tapes in Pool (Status Purged/Expired)')
    tapeGroup.add_argument('-ts', '--tapesInStorage', dest='tapesInStorage', action='store', help='Count how much tapes are in the storage')
...
def checkTape(args):
    ...
    if checkConnection(cursor):
        if args.emptyTapesPool:
            checkResult = checkEmptyTapesPool(cursor, args.warning, args.critical, args.emptyTapesPool)
        if args.emptyTapes:
